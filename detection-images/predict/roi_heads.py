...
class RoIHeads(torch.nn.Module):
    ...
    def check_vaild(self, boxes, labels):
        labels = labels.tolist()
        vaild_indices = []
        for i in range(len(boxes)):
            tag = ''
            min_y, max_y = boxes[i][1], boxes[i][3]
            for j in range(len(self.boundaries)):
                if self.boundaries[j][0] <= min_y and min_y <= self.boundaries[j][1]:
                    tag += str(j)
                    break
            for j in range(len(self.boundaries)):
                if self.boundaries[j][0] <= max_y and max_y <= self.boundaries[j][1]:
                    tag += str(j)
                    break
            if tag in self.types_limit[labels[i]]:
                vaild_indices.append(i)

        return vaild_indices
    ...
    def postprocess_detections(self, class_logits, box_regression, proposals, image_shapes):
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            ...
            # remove boxes in incorrect location
            keep = self.check_vaild(boxes, labels)
            boxes, scores, labels = boxes[keep], scores[keep], labels[keep]
